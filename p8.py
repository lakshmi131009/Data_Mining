...
pathc=os.path.abspath('category-ids.csv')
patha=os.path.abspath('article-ids.csv')

cati=pd.read_csv(pathc,header=None)
...
pathf=os.path.abspath('paths_finished.tsv')
tp=open(pathf)
...
fin['sa']=pd.DataFrame(list(spall.values()))

fin=fin.rename(columns={3:'Category_ID',1:'Number_of_human_paths_traversed','ha':'Number_of_human_times_traversed',
                        'sp':'Number_of_shortest_paths_traversed',
                        'sa':'Number_of_shortest_times_traversed',0:'Category_ID'
                       })
...
fin = fin.reindex(columns= cols)
fin.to_csv('category-paths.csv',index=False)